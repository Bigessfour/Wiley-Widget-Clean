...
public partial class App : Application, IDisposable
{
    ...
    /// </summary>
    public static IServiceProvider ServiceProvider { get; private set; } = null!; // Non-nullable after initialization
    ...
    /// <summary>
    /// Shared splash screen instance created on the UI dispatcher to avoid cross-thread WPF construction.
    /// </summary>
    internal static SplashScreenWindow? SplashScreenInstance { get; private set; }

    /// <summary>
    /// Registers (or clears) the shared splash screen instance.
    /// </summary>
    internal static void SetSplashScreenInstance(SplashScreenWindow? splashScreen)
    {
    ...
    static App()
    {
        // FIRST THING: Create bootstrap logger for early startup logging
        // Only replace the logger if it is still Serilog's silent default; this preserves
        // the richer bootstrap configuration initialized by Program.Main (which includes file sinks).
        ...
        {
            Log.Information("Reusing existing bootstrap logger for App initialization");
        }
    ...
    private static void InitializeDebugInstrumentation()
    {
        ...
        try
        {
            var debugDir = Path.GetDirectoryName(_debugLogPath);
            if (debugDir != null)
            {
    ...
    private static void InitializeBindingDiagnostics()
    {
        try
        {
            ...
            if (listeners.OfType<BindingErrorTraceListener>().Any())
            {
                return;
            }

            PresentationTraceSources.DataBindingSource.Switch.Level = SourceLevels.Warning;
            listeners.Add(new BindingErrorTraceListener());
            Log.Information("WPF binding diagnostics listener attached (warnings and errors will be logged)");
        }
        catch (Exception ex)
        {
            Log.Warning(ex, "Failed to initialize WPF binding diagnostics listener");
        }
    }
    ...
    InitializeBindingDiagnostics();
        ...
        try
        {
            ...
            ConfigureSyncfusionThemes();
            LogDebugEvent("STARTUP_PHASE", "Syncfusion themes configured");

            // Prevent WPF from automatically creating MainWindow
            // We'll create it manually after DI is ready
            this.MainWindow = null;

            // 0. Initial setup (5-15%)
            var phaseStopwatch = System.Diagnostics.Stopwatch.StartNew();
            _splashScreen?.UpdateProgress(5, "Initializing application...");
            ...
            // 1. Show splash screen IMMEDIATELY (perceived performance) (15-25%)
            phaseStopwatch.Restart();
            _splashScreen?.UpdateProgress(15, "Preparing user interface...");
            LogDebugEvent("STARTUP_PHASE", "Phase 1: Splash screen initialization");
            try
            {
                ShowSplashScreen();
                await Task.Delay(200); // Allow splash to render
                _splashScreen?.UpdateProgress(20, "User interface ready...");
            ...
            phaseStopwatch.Restart();
            _splashScreen?.UpdateProgress(25, "Setting up services...");
            ...
            _splashScreen?.UpdateProgress(35, "Configuring services...");
            LogDebugEvent("STARTUP_PHASE", "Phase 2b: Host building");
            
            // Bootstrap logger already created in static constructor
            // ConfigureWpfApplication will replace it with the fully configured logger

            var hostBuilder = Host.CreateApplicationBuilder();
            
            // Configure configuration sources (required for WPF applications using Generic Host)
            ...
            {
                viewManager.RegisterExistingWindow(splash);
            }

            // Replace bootstrap logger with fully configured logger
            var config = ServiceProvider.GetRequiredService<IConfiguration>();
            var serilogLogger = new Serilog.LoggerConfiguration().ReadFrom.Configuration(config).CreateLogger();
            Serilog.Log.Logger = serilogLogger;

            // Microsoft pattern: Get structured logger from DI container after host.Build()
            var logger = ServiceProvider.GetRequiredService<Microsoft.Extensions.Logging.ILogger<App>>();
            logger.LogInformation("Host built successfully - structured logging now available from DI container");
            
            // Verify that Serilog configuration from appsettings.json took effect
            Log.Information("Host configured - Serilog configuration loaded from appsettings.json");
            logger.LogInformation("Microsoft Extensions Logging integration successful - using Serilog from configuration");
            ...
            {
                logger.LogInformation("Serilog configuration section found - Microsoft pattern integration successful");
                Log.Information("Configuration integration verified: Serilog section loaded with {MinLevel} minimum level", 
                    serilogSection["MinimumLevel:Default"]);
            }
            else
            {
                logger.LogWarning("Serilog configuration section not found - using default configuration");
            }

            // Log resolved database connection information for diagnostics without exposing credentials
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(resolvedConnection))
                {
                    ...
                    logger.LogInformation("Database connection resolved for {Environment}: {DataSource}/{InitialCatalog}",
                        hostEnvironment,
                        builder.DataSource,
                ...
                {
                    logger.LogWarning("Resolved database connection string was empty for environment {Environment}", hostEnvironment);
                }
            }
            catch (Exception resolveEx)
            {
                logger.LogWarning(resolveEx, "Failed to resolve database connection string for environment {Environment}", hostEnvironment);
            }

            // Essential service initialization moved to HostedWpfApplication service
            _splashScreen?.UpdateProgress(65, "Initializing components...");
            Log.Information("Service initialization delegated to HostedWpfApplication service");

            // ❌ REMOVED: Database initialization duplication - now handled by BackgroundInitializationService
            // BackgroundInitializationService.ExecuteAsync() performs comprehensive database initialization:
            // 1. EnsureDatabaseCreatedAsync - creates database if missing
            // 2. ValidateDatabaseSchemaAsync - verifies schema integrity
            // This runs in parallel with UI startup for optimal performance
            _splashScreen?.UpdateProgress(75, "Starting services...");
            Log.Information("📊 Database initialization delegated to BackgroundInitializationService - ID: {StartupId}", startupId);

            // Start the host (HostedWpfApplication will create and show MainWindow)
            phaseStopwatch.Restart();
            _splashScreen?.UpdateProgress(80, "Starting application...");
            ...
            {
                // Create MainWindow on STA thread before base.OnStartup
                var mainWindowViewManager = ServiceProvider.GetService<IViewManager>();
                if (mainWindowViewManager != null)
                {
                    await mainWindowViewManager.ShowMainWindowAsync(CancellationToken.None);
                    Log.Information("MainWindow created successfully via ViewManager [{StartupId}]", startupId);
                }
                else
                {
                    Log.Warning("ViewManager not available for MainWindow creation [{StartupId}]", startupId);
                }
                ...
                // VERBOSE INITIALIZATION CONFIRMATION LOGGING
                Log.Information("=== FULL APPLICATION INITIALIZATION CONFIRMATION - ID: {StartupId} ===", startupId);
                
                // Verify MainWindow creation
                if (this.MainWindow != null)
                {
                    Log.Information("✓ MainWindow created successfully - Type: {WindowType}, Title: {Title}, Visibility: {Visibility}",
                        this.MainWindow.GetType().Name, this.MainWindow.Title, this.MainWindow.Visibility);
                    Log.Debug("MainWindow details - Width: {Width}, Height: {Height}, WindowState: {WindowState}",
                        this.MainWindow.Width, this.MainWindow.Height, this.MainWindow.WindowState);
                }
                else
                {
                    Log.Warning("⚠ MainWindow is null after startup completion - this may indicate initialization issues");
                }
                ...
                Log.Information("✓ System resources - Memory: {MemoryMB}MB, Threads: {ThreadCount}, Handles: {HandleCount}",
                    process.WorkingSet64 / 1024 / 1024, process.Threads.Count, process.HandleCount);

                // Log application state
                Log.Information("✓ Application state - ShutdownMode: {ShutdownMode}, CurrentCulture: {Culture}, CurrentUICulture: {UICulture}",
                    this.ShutdownMode, Thread.CurrentThread.CurrentCulture.Name, Thread.CurrentThread.CurrentUICulture.Name);

                // THREAD DIAGNOSTICS LOGGING
                Log.Debug("🧵 Thread diagnostics - Current Thread: ID={ThreadId}, ApartmentState: {ApartmentState}, IsBackground: {IsBackground}",
                    Thread.CurrentThread.ManagedThreadId, Thread.CurrentThread.GetApartmentState(), Thread.CurrentThread.IsBackground);

                // Monitor for thread state changes during application lifetime
                this.Dispatcher.Hooks.DispatcherInactive += (sender, args) =>
                {
                    Log.Verbose("🧵 Dispatcher inactive - UI thread processing completed");
                };

                this.Dispatcher.Hooks.OperationPosted += (sender, args) =>
                {
                    Log.Verbose("🧵 Dispatcher operation posted - Operation: {Operation}",
                        args.Operation?.GetType().Name ?? "Unknown");
                };

                Log.Information("=== APPLICATION FULLY INITIALIZED AND READY - ID: {StartupId} ===", startupId);

                // SETUP WPF LIFECYCLE EVENT LOGGING
                if (this.MainWindow != null)
                {
                    this.MainWindow.ContentRendered += (sender, args) =>
                    {
                        Log.Information("🎨 MainWindow ContentRendered - UI fully painted and ready for user interaction");
                        Log.Debug("MainWindow render details - ActualWidth: {Width}, ActualHeight: {Height}, RenderSize: {RenderSize}",
                            this.MainWindow.ActualWidth, this.MainWindow.ActualHeight, this.MainWindow.RenderSize);
                    };

                    this.MainWindow.Activated += (sender, args) =>
                    {
                        Log.Debug("�️ MainWindow Activated - Window gained focus");
                    };

                    this.MainWindow.Deactivated += (sender, args) =>
                    {
                        Log.Debug("🖼️ MainWindow Deactivated - Window lost focus");
                    };

                    this.MainWindow.SizeChanged += (sender, args) =>
                    {
                        Log.Verbose("📏 MainWindow SizeChanged - NewSize: {Width}x{Height}, PreviousSize: {PrevWidth}x{PrevHeight}",
                            args.NewSize.Width, args.NewSize.Height, args.PreviousSize.Width, args.PreviousSize.Height);
                    };

                    this.MainWindow.LocationChanged += (sender, args) =>
                    {
                        Log.Verbose("📍 MainWindow LocationChanged - Position: ({Left}, {Top})",
                            this.MainWindow.Left, this.MainWindow.Top);
                    };

                    this.MainWindow.Closing += (sender, args) =>
                    {
                        Log.Information("🚪 MainWindow Closing - User initiated shutdown, Cancel: {Cancel}",
                            args.Cancel);
                    };

                    this.MainWindow.Closed += (sender, args) =>
                    {
                        Log.Information("🚪 MainWindow Closed - Window has been closed");
                    };

                    Log.Debug("🎯 WPF lifecycle event handlers attached to MainWindow");
                }

                this.SessionEnding += (sender, args) =>
                {
                    Log.Warning("🔌 Session Ending - Reason: {ReasonCode}, System is shutting down",
                        args.ReasonSessionEnding);
                };

                Log.Debug("🎯 WPF application lifecycle event handlers registered");
        ...
        {
            Log.Fatal(ex, "Unhandled exception during application startup after {ElapsedMs}ms [{StartupId}]", 
                startupStopwatch.ElapsedMilliseconds, startupId);
        ...
        finally
        {
            ...
            Log.Information("Startup sequence completed");
            // Microsoft pattern: Don't close and flush here, let OnExit handle it
        }
    }
    ...
    protected override void OnExit(ExitEventArgs e)
    {
        try
        {
            ...
            // Stop the host gracefully before disposing
            if (_host != null)
            {
                Log.Information("Stopping host services gracefully");
                try
                {
                ...
                {
                    Log.Warning(stopEx, "Failed to stop host gracefully, proceeding with dispose");
                }
            }
        ...
        {
            // Microsoft pattern: Always close and flush logging in finally block
    ...
    /// <summary>
    /// Implements IDisposable to properly dispose of owned resources.
    /// </summary>
    public void Dispose()
    {
        Dispose(true);
    ...
    /// <summary>
    /// Cleans up orphaned .NET processes that may have been left running
    /// </summary>
        ...
        try
        {
            ...
            if (parentId.HasValue)
            {
                try
                {
                    ...
                    if (parent.ProcessName.Equals("dotnet", StringComparison.OrdinalIgnoreCase) ||
                        parent.ProcessName.Equals("devenv", StringComparison.OrdinalIgnoreCase))
                    {
                        Log.Information("Skipping orphan cleanup while running under host process {Parent}", parent.ProcessName);
                        return;
                    }
                }
                catch
                {
                    // Parent exited—treat as orphan cleanup scenario.
                }
            }
            ...
            {
                Log.Information("Skipping orphaned process cleanup because application is running via dotnet host ({ProcessPath})",
                    processPath);
                return;
            }

            Log.Information("Checking for orphaned .NET processes to clean up");
            
            var currentProcessId = currentProcess.Id;
            var currentProcessName = currentProcess.ProcessName;
            
            var orphanedProcesses = System.Diagnostics.Process.GetProcesses()
                .Where(p => 
                    p.ProcessName.Contains("dotnet") || 
                    p.ProcessName.Contains("WileyWidget") ||
                    p.ProcessName.Contains("Microsoft.NET"))
                .Where(p => p.Id != currentProcessId)
                .Where(p => 
                {
                    try
                    {
                        // Check if process has no parent (orphaned) or parent is not our process
                        var parentId = GetParentProcessId(p.Id);
                        return parentId == null || parentId != currentProcessId;
                    }
                    catch
                    {
                        // If we can't get parent, assume it's orphaned
                        return true;
                    }
                })
                .ToList();
            ...
            {
                Log.Warning("Found {Count} orphaned .NET processes, cleaning up", orphanedProcesses.Count);
                
                foreach (var process in orphanedProcesses)
                {
                    try
                    {
                        Log.Information("Terminating orphaned process: {Name} (PID: {Id})", process.ProcessName, process.Id);
                        process.Kill();
                        process.WaitForExit(2000); // Wait up to 2 seconds
                    }
                    catch (Exception ex)
                    {
                        Log.Warning(ex, "Failed to terminate orphaned process {Name} (PID: {Id})", process.ProcessName, process.Id);
                    }
                }
            }
            else
            {
                Log.Information("No orphaned .NET processes found");
            }
        }
        catch (Exception ex)
        {
    ...
    /// </summary>
#pragma warning disable CA1416 // ManagementObjectSearcher is Windows-only, but this is a Windows WPF app
    ...
    /// Must be called BEFORE any WPF windows are created to ensure proper theme application
    /// 
    /// SUCCESS CRITERIA:
    /// - FluentDark theme successfully applied as application theme
    /// - ApplyThemeAsDefaultStyle enabled for automatic control styling
    /// - No exceptions during theme configuration
    /// - Verification logging confirms theme is active
    /// </summary>
    private void ConfigureSyncfusionThemes()
    {
        var themeTimer = System.Diagnostics.Stopwatch.StartNew();
        Log.Information("🎨 [CRITICAL] Starting Syncfusion theme configuration");
        LogDebugEvent("SYNCFUSION_THEMES", "Theme configuration started");
        
        try
        {
            // ✅ CRITICAL OPERATION: Set application-wide theme
            Log.Debug("🎨 Setting SfSkinManager.ApplicationTheme to FluentDark");
            SfSkinManager.ApplicationTheme = new Theme("FluentDark");
            LogDebugEvent("SYNCFUSION_THEMES", "ApplicationTheme set to FluentDark");

            // ✅ CRITICAL OPERATION: Enable automatic theme application
            Log.Debug("🎨 Enabling SfSkinManager.ApplyThemeAsDefaultStyle");
            SfSkinManager.ApplyThemeAsDefaultStyle = true;
            LogDebugEvent("SYNCFUSION_THEMES", "ApplyThemeAsDefaultStyle enabled");

            themeTimer.Stop();
            
            // ✅ VERIFICATION: Confirm successful theme configuration
            Log.Information("✅ [SUCCESS] Syncfusion themes configured in {ElapsedMs}ms", themeTimer.ElapsedMilliseconds);
            Log.Information("   ➜ Active Theme: FluentDark");
            Log.Information("   ➜ Auto-apply enabled: All Syncfusion controls will use FluentDark theme");
            Log.Information("   ➜ Theme will be applied before any windows are created");
            LogDebugEvent("SYNCFUSION_THEMES", $"Configuration succeeded (completed in {themeTimer.ElapsedMilliseconds}ms)");
            LogStartupTiming("Syncfusion Theme Configuration", themeTimer.Elapsed, "FluentDark theme active");
        }
        catch (Exception ex)
        {
            themeTimer.Stop();
            Log.Error(ex, "❌ [FAILURE] Syncfusion theme configuration FAILED after {ElapsedMs}ms", themeTimer.ElapsedMilliseconds);
            Log.Error("   ➜ Application will use DEFAULT THEME (controls may appear unthemed)");
            Log.Error("   ➜ Visual appearance may be inconsistent or missing styling");
            Log.Error("   ➜ Exception details: {ExceptionType}: {ExceptionMessage}", ex.GetType().Name, ex.Message);
            LogDebugEvent("SYNCFUSION_THEMES", $"Configuration failed: {ex.Message}");
            
            // Non-fatal: allow app to continue with default theming
        }
    }
    ...
    private void ConfigureGlobalExceptionHandling()
    {
        AppDomain.CurrentDomain.UnhandledException += (sender, args) =>
        {
            ...
            Log.Fatal(ex, "💀 CRITICAL: AppDomain Unhandled Exception - Application will terminate. IsTerminating: {IsTerminating}, ExceptionType: {ExceptionType}",
                args.IsTerminating, ex.GetType().Name);

            // Ensure logging even if ErrorReportingService fails
            try
            {
        ...
        {
            Log.Error(args.Exception, "🚨 Dispatcher Unhandled Exception - ExceptionType: {ExceptionType}, Handled: {Handled}",
                args.Exception.GetType().Name, args.Handled);
            ...
            {
                Log.Error(reportEx, "❌ FAILED to report Dispatcher exception via ErrorReportingService");
            }
        ...
        {
            Log.Warning(args.Exception, "⚠️ Unobserved Task Exception - ExceptionType: {ExceptionType}, Observed: {Observed}",
                args.Exception.GetType().Name, args.Observed);
            ...
            {
                Log.Warning(reportEx, "❌ FAILED to report Task exception via ErrorReportingService");
            }

            args.SetObserved();
        };

        // Additional safety: Log that exception handlers are registered
        Log.Debug("🛡️ Global exception handlers registered - AppDomain, Dispatcher, and TaskScheduler");
    }
    ...
    private void ShowSplashScreen()
    {
        ...
        try
        {
            ...
            if (ServiceProvider?.GetService<IViewManager>() is IViewManager viewManager)
            {
                ...
                // ✅ FIX: Call on main thread, not background thread
                _ = viewManager.ShowSplashScreenAsync(System.Threading.CancellationToken.None);
            ...
            else
            {
                ...
                SetSplashScreenInstance(_splashScreen);
                _splashScreen.Show();
