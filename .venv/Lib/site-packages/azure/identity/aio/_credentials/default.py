...
class DefaultAzureCredential(ChainedTokenCredential):
    ...
    def __init__(
        ...
        exclude_flags = process_credential_exclusions(
            credential_config, exclude_flags, user_excludes
        )
        ...
        if not exclude_workload_identity_credential:
            try:
                ...
                credentials.append(
                    AsyncFailedDACCredential(
                        "WorkloadIdentityCredential", error=str(ex)
                    )
                )
    ...
    async def get_token(
        ...
        **kwargs: Any
        ...
        if self._successful_credential:
            token = await cast(
                AsyncTokenCredential, self._successful_credential
            ).get_token(*scopes, claims=claims, tenant_id=tenant_id, **kwargs)
            _LOGGER.info(
    ...
    async def get_token_info(
        ...
        """
        if self._successful_credential:
            token_info = await cast(
                AsyncSupportsTokenInfo, self._successful_credential
            ).get_token_info(*scopes, options=options)
            _LOGGER.info(
                "%s acquired a token from %s",
                self.__class__.__name__,
                self._successful_credential.__class__.__name__,
        ...
        within_dac.set(True)
        try:
            token_info = await cast(AsyncSupportsTokenInfo, super()).get_token_info(
