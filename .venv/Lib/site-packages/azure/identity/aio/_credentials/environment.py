...
class EnvironmentCredential(AsyncContextManager):
    ...
    def __init__(self, **kwargs: Any) -> None:
        self._credential: Optional[
            Union[CertificateCredential, ClientSecretCredential]
        ] = None

        if all(
            os.environ.get(v) is not None
            for v in EnvironmentVariables.CLIENT_SECRET_VARS
        ):
            ...
            self._credential = CertificateCredential(
                client_id=os.environ[EnvironmentVariables.AZURE_CLIENT_ID],
                tenant_id=os.environ[EnvironmentVariables.AZURE_TENANT_ID],
                certificate_path=os.environ[
                    EnvironmentVariables.AZURE_CLIENT_CERTIFICATE_PATH
                ],
                password=os.environ.get(
                    EnvironmentVariables.AZURE_CLIENT_CERTIFICATE_PASSWORD
                ),
                send_certificate_chain=bool(
                    os.environ.get(
                        EnvironmentVariables.AZURE_CLIENT_SEND_CERTIFICATE_CHAIN, False
                    )
                ),
                **kwargs
            )
        ...
        if self._credential:
            _LOGGER.info(
                "Environment is configured for %s", self._credential.__class__.__name__
            )
        else:
            expected_variables = set(
                EnvironmentVariables.CERT_VARS + EnvironmentVariables.CLIENT_SECRET_VARS
            )
    ...
    async def get_token_info(
        ...
        if not self._credential:
            ...
            raise CredentialUnavailableError(message=message)
        return await cast(AsyncSupportsTokenInfo, self._credential).get_token_info(
