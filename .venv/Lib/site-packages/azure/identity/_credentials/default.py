...
class DefaultAzureCredential(ChainedTokenCredential):
    ...
    def __init__(
        ...
        broker_tenant_id = kwargs.pop(
            "broker_tenant_id", os.environ.get(EnvironmentVariables.AZURE_TENANT_ID)
        )
        ...
        if not exclude_workload_identity_credential:
            try:
                ...
                credentials.append(
                    FailedDACCredential("WorkloadIdentityCredential", error=str(ex))
                )
    ...
    def get_token_info(
        ...
        if self._successful_credential:
            token_info = cast(
                SupportsTokenInfo, self._successful_credential
        ...
        within_dac.set(True)
        try:
            token_info = cast(SupportsTokenInfo, super()).get_token_info(
