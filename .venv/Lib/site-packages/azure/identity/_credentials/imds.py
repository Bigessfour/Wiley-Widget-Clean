...
class ImdsRetryPolicy(RetryPolicy):
    ...
    def is_retry(
        self, settings: Dict[str, Any], response: PipelineResponse[Any, Any]
    ) -> bool:
...
class ImdsCredential(MsalManagedIdentityClient):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(
            retry_policy_class=ImdsRetryPolicy, **dict(PIPELINE_SETTINGS, **kwargs)
        )
        self._config = kwargs
        ...
        if within_credential_chain.get() and not self._endpoint_available:
            ...
            try:
                client = ManagedIdentityClient(
                    _get_request, **dict(PIPELINE_SETTINGS, **self._config)
                )
