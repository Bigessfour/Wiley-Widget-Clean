...
TROUBLESHOOTING_GUIDE = (
    "https://aka.ms/azsdk/python/identity/azurepipelinescredential/troubleshoot"
)
...
class AzurePipelinesCredential:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if (
            not system_access_token
            or not tenant_id
            or not client_id
            or not service_connection_id
        ):
            raise ValueError(
    ...
    def get_token_info(
        ...
        """
        validate_env_vars()
        return self._client_assertion_credential.get_token_info(
            *scopes, options=options
        )

    def _get_oidc_token(self) -> str:
        request = build_oidc_request(
            self._service_connection_id, self._system_access_token
        )
        ...
        if http_response.status_code not in [200]:
            raise ClientAuthenticationError(
                message="Unexpected response from OIDC token endpoint.",
