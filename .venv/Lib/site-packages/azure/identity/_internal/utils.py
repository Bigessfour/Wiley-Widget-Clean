...
def validate_subscription(subscription: str) -> None:
    ...
    """
    if not subscription or any(
        c not in VALID_SUBSCRIPTION_CHARACTERS for c in subscription
    ):
        raise ValueError(
...
def process_credential_exclusions(
    credential_config: dict, exclude_flags: dict, user_excludes: dict
) -> dict:
    ...
    if token_credentials_env == "dev":
        ...
        valid_credentials = {
            config["env_name"]
            for config in credential_config.values()
