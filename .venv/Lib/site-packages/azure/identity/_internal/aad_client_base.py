...
class AadClientBase(abc.ABC):
    ...
    def get_cached_refresh_tokens(self, scopes: Iterable[str], **kwargs) -> List[Dict]:
        ...
        cache = self._get_cache(**kwargs)
        return list(
            cache.search(TokenCache.CredentialType.REFRESH_TOKEN, target=list(scopes))
        )
    ...
    def _process_response(
        ...
        refresh_on = (
            request_time + int(content["refresh_in"])
            if "refresh_in" in content
            else None
        )
    ...
    def _get_client_certificate_assertion(
        ...
        jws = (
            base64.urlsafe_b64encode(jwt_header)
            + b"."
            + base64.urlsafe_b64encode(payload)
        )
        signature = (
            certificate.sign_ps256(jws)
            if not self._is_adfs
            else certificate.sign_rs256(jws)
        )
