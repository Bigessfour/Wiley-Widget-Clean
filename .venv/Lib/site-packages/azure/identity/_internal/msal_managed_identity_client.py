...
class MsalManagedIdentityClient(
    abc.ABC
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._msal_client = msal.ManagedIdentityClient(
            managed_identity, http_client=self._client
        )
    ...
    ) -> AccessTokenInfo:  # pylint:disable=unused-argument
        if not scopes:
    ...
    def get_managed_identity(
        ...
        if "client_id" in self._settings and self._settings["client_id"]:
            return msal.UserAssignedManagedIdentity(
        ...
        if "resource_id" in identity_config and identity_config["resource_id"]:
            return msal.UserAssignedManagedIdentity(
                resource_id=identity_config["resource_id"]
            )
        if "object_id" in identity_config and identity_config["object_id"]:
            return msal.UserAssignedManagedIdentity(
                object_id=identity_config["object_id"]
            )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise ClientAuthenticationError(
                self.get_unavailable_message(str(ex))
            ) from ex
