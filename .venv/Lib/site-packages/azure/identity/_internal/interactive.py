...
class InteractiveCredential(MsalCredential, ABC):
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self._auth_record:
            kwargs.pop(
                "client_id", None
            )  # authentication_record overrides client_id argument
    ...
    def get_token(
        ...
        **kwargs: Any,
        ...
        token_info = self._get_token_base(
            *scopes, options=options, base_method_name="get_token", **kwargs
        )
    ...
    def get_token_info(
        ...
        :raises AuthenticationRequiredError: user interaction is necessary to acquire a token, and the credential is
            configured not to begin this automatically. Call :func:`authenticate` to begin interactive authentication.
        """
        return self._get_token_base(
            *scopes, options=options, base_method_name="get_token_info"
        )
    ...
    def _get_token_base(
        ...
        **kwargs: Any,
        ...
        if not scopes:
            ...
            _LOGGER.warning(
                "%s.%s failed: %s", self.__class__.__name__, base_method_name, message
            )
            raise ValueError(message)

        allow_prompt = kwargs.pop(
            "_allow_prompt", not self._disable_automatic_authentication
        )
        options = options or {}
        ...
        for key in options:
            if (
                key not in TokenRequestOptions.__annotations__
        ...
        if self._auth_record:
            ...
            for account in app.get_accounts(username=self._auth_record.username):
                ...
                if result and "access_token" in result and "expires_in" in result:
                    refresh_on = (
                        int(result["refresh_on"]) if "refresh_on" in result else None
                    )
                    return AccessTokenInfo(
                        result["access_token"],
                        now + int(result["expires_in"]),
