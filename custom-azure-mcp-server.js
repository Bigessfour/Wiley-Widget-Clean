    ...
    inputSchema: {
      ...
      properties: {
        ...
        resource_type: {
          type: "string",
          description:
            "Optional resource type to filter by (e.g., Microsoft.Storage/storageAccounts)",
        },
      },
    },
  },
...
class AzureMCPServer {
  ...
  async handleResourceList(args) {
    ...
    const result = await this.runAzureCommand(
      commandArgs[0],
      commandArgs.slice(1),
    );
      ...
      content: [
        {
          ...
          text: `Found ${resources.length} Azure resources:\n\n${resources
            .map(
              (r) => `- ${r.name} (${r.type}) in ${r.resourceGroup || "N/A"}`,
            )
            .join("\n")}`,
        },
      ],
  ...
  async handleResourceGroupsList() {
    const result = await this.runAzureCommand("group", [
      "list",
      "--output",
      "json",
    ]);
      ...
      content: [
        {
          ...
          text: `Found ${accounts.length} storage accounts:\n\n${accounts
            .map((a) => `- ${a.name} (${a.kind}, ${a.location})`)
            .join("\n")}`,
        },
      ],
    };
  ...
  async handleAccountShow() {
    const result = await this.runAzureCommand("account", [
      "show",
      "--output",
      "json",
    ]);
