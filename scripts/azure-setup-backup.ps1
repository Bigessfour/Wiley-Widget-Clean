...
if ($CreateResources) {
    ...
    az group create --name "WileyWidget-RG" --location "East US 2" --output none

    # Create SQL Server
    Write-Host "  Creating Azure SQL Server..." -ForegroundColor Yellow
    az sql server create --name $env:AZURE_SQL_SERVER.Split('.')[0] --resource-group "WileyWidget-RG" --location "East US 2" --admin-user $env:AZURE_SQL_USER --admin-password $env:AZURE_SQL_PASSWORD --output none
...
Write-Host "  • Test connection: .\scripts\azure-setup.ps1 -TestConnection" -ForegroundColor Gray
Write-Host "  • Create resources: .\scripts\azure-setup.ps1 -CreateResources" -ForegroundColor Gray
Write-Host "  • Deploy database: .\scripts\azure-setup.ps1 -DeployDatabase" -ForegroundColor Gray
